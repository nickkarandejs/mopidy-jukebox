...
package %w{libsqlite3-dev libffi-dev gstreamer1.0-fluendo-mp3 gstreamer1.0-plugins-bad gstreamer1.0-alsa gstreamer1.0-plugins-good gstreamer1.0-plugins-ugly libssl-dev gir1.2-gstreamer-1.0 gir1.2-gst-plugins-base-1.0 python-dev python-gi cron git} do
	action :install
end
...
cron 'cache update' do
  ...
  command "/usr/local/bin/mopidy --config /etc/mopidy/mopidy.conf cacher 1>/root/mopidy-cacher.log 2>&1"
end
...
cron 'local scan' do
  ...
  user 'root'
  mailto 'palfrey@lshift.net'
  command "/usr/local/bin/mopidy --config /etc/mopidy/mopidy.conf local scan 1>/root/mopidy-scan.log 2>&1"
end
